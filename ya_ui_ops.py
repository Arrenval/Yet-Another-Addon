...
class UI_OT_YA_ConsoleToolsDirectory(Operator):
    ...
    def execute(self, context):
        ...
        if os.path.exists(selected_file) and selected_file.endswith("ConsoleTools.exe"):
            ...
            context.scene.ya_props.textools_directory = textools_folder
...
class UI_OT_YA_PMPSelector(Operator):
    ...
    def invoke(self, context, event):
        actual_file = context.scene.ya_props.loadmodpack_directory 
    ...
    def execute(self, context):
        ...
        if os.path.exists(selected_file) and selected_file.endswith(".pmp"):
            ...
            context.scene.ya_props.loadmodpack_directory = selected_file
            display_dir = utils.directory_short(selected_file, 1) 
            context.scene.ya_props.loadmodpack_display_directory = display_dir[:-4]

...
def modpack_data(context) -> None:
    scene = context.scene
    scene.loaded_pmp_groups.clear()

    blender_groups = scene.pmp_mod_groups
    modpack_path = Path(scene.file_props.modpack_dir)
    ...
    for idx, group in enumerate(modpack.groups):
        new_option = context.scene.file_props.loaded_pmp_groups.add()
    ...
    scene.file_props.modpack_author  = modpack.meta.Author
    scene.file_props.modpack_version = modpack.meta.Version
...
class YAFileProps(PropertyGroup):
    ...
    def extra_options() -> None:
        for (name, category, default, description) in YAFileProps.extra_buttons_list:
    ...
    @staticmethod
    def ui_buttons() -> None:
        for (name, category, description) in YAFileProps.ui_buttons_list:
            category_lower = category.lower()
            name_lower = name.lower()
    ...
    loaded_pmp_groups: CollectionProperty(type=LoadedModpackGroup) # type: ignore
...
def set_addon_properties() -> None:
    ...
    YAFileProps.ui_buttons()
    YAFileProps.extra_options()
